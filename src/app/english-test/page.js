    ...
    >
      {englishTest.length > 0 ? (
        <div className="container mx-auto bg-slate-600 px-20 py-10 w-1/2 bg-violet-300 ">
          <h1 className="pb-6 font-bold text-2xl">English Test</h1>
          <div></div>
          <div>
            {!displayResult ? (
              <div className="">
                <h2 className="text-3xl font-bold py-2 mb-4">
                  {currentQue + 1}. {englishTest[currentQue].question}
                </h2>
                <ul>
                  {englishTest[currentQue].answers.map((enAnswer, index) => (
                    <>
                      <li
                        key={index}
                        onClick={(event) => {
                          ...
                          if (
                            enAnswer ===
                              englishTest[currentQue].correctAnswer &&
                            checkSelected === false
                          ) {
                ...
                {currentQue < 5 ? (
                  <div className="">
                    <button
                      className="py-2 px-6 bg-violet-100 font-bold text-2xl mt-8"
                      onClick={() => {
              ...
              <div className="">
                ...
                <div className="">
                  From {englishTest.length} Questions you answer
                  <span className="result bg-black py-2 px-4 text-yellow-100">
                    {result}
                  </span>{' '}
                  truly and your score is {result * (100 / englishTest.length)}
                  %
                </div>
                <Link href="/">Choose Another Test!</Link>
              </div>
